/**
 * Pre-Setup Script for IGP131 Payload Simulation
 *
 * 1. Mock Chainlink feed 0x66ac... so latestRoundData() returns fixed values.
 * 2. Deploy PST-USDC DEX 45 and vaults 165-169 if the fork predates them
 *    (needed for action 4 PST launch limits).
 */
...
function readPstAddressFromConstants(): string {
  ...
  if (match[1] === "address(0)") {
    throw new Error(
      "PST_ADDRESS is still address(0). Fill the real PST mainnet address before simulating PST vault deployment.",
    );
  }
...
function getDeployVaultCalldata(
  ...
  const perType = {
    ...
    T3: {
      ...
      signature:
        "function vaultT3(address supplyToken_, address smartDebt_) external",
      method: "vaultT3",
    },
    T4: {
...
async function ensureDex45(
  ...
  if (!(await hasCode(provider, dex45))) {
    throw new Error(
      `DEX ${PST_USDC_DEX_ID} deployment did not create code at ${dex45}`,
    );
  }
  return dex45;
}
...
async function ensurePstEcosystem(provider: JsonRpcProvider): Promise<void> {
  ...
  if (!(await hasCode(provider, usdcUsdtDex))) {
    throw new Error(
      `Required USDC-USDT DEX ${USDC_USDT_DEX_ID} has no code at ${usdcUsdtDex}`,
    );
  }
...
export async function preSetup(
  ...
  console.log("[SETUP] Running pre-setup for IGP131...");

  try {
    await mockChainlinkFeed(provider);
    await ensurePstEcosystem(provider);
