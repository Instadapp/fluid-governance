...
contract PayloadIGP105 is PayloadIGPMain {
    ...
    function action9() internal isActionSkippable(9) {
        // Step 1: Withdraw 55 stETH from Lite vault to Treasury
        {
            ...
            string
                memory withdrawSignature = "withdraw(address,uint256,uint256,uint256)";
        ...
        // Step 2: Transfer 55 stETH from Treasury DSA to Lite vault's address
        {
            string[] memory targets = new string[](1);
            bytes[] memory encodedSpells = new bytes[](1);
            ...
            // Spell 1: Transfer 55 stETH to Lite vault
            {
                uint256 STETH_AMOUNT = 55 * 1e18; // 55 stETH
                targets[0] = "BASIC-A";
                encodedSpells[0] = abi.encodeWithSignature(
