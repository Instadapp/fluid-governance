...
/// @notice IGP-X (staging): Liquidity Layer UserModule and AdminModule upgrades with
///         rollback registration and pause / rates / range auth rotations.
///         Split from IGP-133; assign PROPOSAL_ID before governance propose.
contract PayloadIGPX is PayloadIGPPriceHelpers {
    /// @dev Update when this payload is scheduled on-chain.
    ...
    function action1() internal isActionSkippable(1) {
        address newUserModule_ = PayloadIGPX(ADDRESS_THIS).newUserModuleAddress();
    ...
    function action3() internal isActionSkippable(3) {
        address newAdminModule_ = PayloadIGPX(ADDRESS_THIS).newAdminModuleAddress();
    ...
    function action5() internal isActionSkippable(5) {
        address liquidityPauseAuth_ = PayloadIGPX(ADDRESS_THIS).liquidityPauseAuth();
        address dexPauseAuth_ = PayloadIGPX(ADDRESS_THIS).dexPauseAuth();
    ...
    function action6() internal isActionSkippable(6) {
        address newRatesAuth_ = PayloadIGPX(ADDRESS_THIS).newRatesAuth();
    ...
    function action7() internal isActionSkippable(7) {
        address newRangeAuth_ = PayloadIGPX(ADDRESS_THIS).newRangeAuth();
