 ...
 *
 * 1. Deploy the USDai vaults (ids 174-179) the payload configures.
 *    IGP-134 raises launch limits on vaults 171-179, but only 171-173 exist on
 *    the base mainnet fork unless IGP-133's deploys have landed. The remaining
 *    six (174-179) were queued as pending "deploy only, no config" transactions
 *    on the Avocado deployer multisig
 *    (0x1e2e1aeD876f67Fe4Fd54090FD7B8F57Ce234219). We replay the exact
 *    `deployVault(logic, vaultData)` calldata of those pending transactions
 *    against the VaultFactory from the deployer safe (in ascending vaultId
 *    order, since the factory assigns the next sequential id on each deploy) so
 *    the fork matches the post-deployment mainnet state the payload assumes.
 *    Already-deployed vaults are skipped.
 *
 ...
 *
 * 3. Set liteStethRevenueAmount on the payload for Action 2.
 *    Action 2 reverts unless Team Multisig configures a non-zero stETH amount.
...
// Non-zero stETH wei for Action 2 (iETHv2 revenue claim).
...
export async function preSetup(
  ...
  try {
    ...
    // Step 3: set the iETHv2 revenue amount for Action 2.
    if (payloadAddress) {
